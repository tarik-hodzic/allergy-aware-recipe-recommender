...
def fuzzy_normalize(word, vocabulary, threshold=80):
    ...
    Ako nema pogodaka (extractOne vrati None), vraca originalnu rijec.
    """
    # ako je word None ili prazan, samo ga vrati
...
def normalize_user_ingredients(text, vocabulary):
    """
    1) Spell-check unosa korisnika
    2) Fuzzy normalizacija prema TF-IDF vokabularu
    Osiguravamo da uvijek vracamo samo stringove (bez None).
    """
    ...
    for w in words:
        ...
        # ako spellchecker vrati None, zadrzi originalnu rijec
    ...
    for w in corrected:
        if not isinstance(w, str) or not w:
            continue
        n = fuzzy_normalize(w, vocabulary)
...
def normalize_text(text):
    ...
    # 2) proba dekodirati unicode escape (\u00bd, \u00bc...)
    try:
...
def recommend_recipes(user_ingredients: str, user_allergens, top_k: int):
    ...
    def count_overlap(row):
        recipe_words = set(row["ingredients_tokens"])  # lemmatized tokens
